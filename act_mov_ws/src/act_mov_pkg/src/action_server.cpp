...
public:
  NavigateActionServerNode() : Node("navigate_action_server_node")
  {
    ...
    robot_position_subscription_ = this->create_subscription<Point>(
        "robot_position", 10, std::bind(&NavigateActionServerNode::update_robot_position, this, std::placeholders::_1));
...
private:
  ...
  {
    (void)uuid;
    std::cout << "recieved goal point: (" << goal->goal_point.x << "," << goal->goal_point.y << ","
  ...
  void execute(const std::shared_ptr<GoalHandle> goal_handle)
  {
    ...
    {
      feedback->distance_to_point = std::sqrt(std::pow(this->robot_position_.x - goal->goal_point.x, 2) +
